...
{counter name="fieldsUsed"}
<td width='12.5%' scope="col">
{if !$fields.tog_company_contacts_name.hidden}
...
{if !empty($fields.tog_company_contactstog_company_ida.value)}
{capture assign="detail_url"}index.php?module=tog_Company&action=DetailView&record={$fields.tog_company_contactstog_company_ida.value}{/capture}
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="tog_company_contactstog_company_ida" class="sugar_field" data-id-value="{$fields.tog_company_contactstog_company_ida.value}">{$fields.tog_company_contacts_name.value}</span>
{if !empty($fields.tog_company_contactstog_company_ida.value)}</a>{/if}
{/if}
</td>
</tr>
{/capture}
